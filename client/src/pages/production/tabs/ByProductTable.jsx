...
const ByProductTable = ({ items, onRemove }) => {
  // Calculate Totals
  const totals = items.reduce((acc, item) => ({
    quantity: acc.quantity + parseFloat(item.quantity || 0),
    pieces: acc.pieces + parseFloat(item.pieces || 0),
    value: acc.value + parseFloat(item.value || 0),
  }), { quantity: 0, pieces: 0, value: 0 });
          ...
          <table className="w-full text-left text-[11px]">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                ...
                <th className="border-b border-r border-slate-200 px-3 py-2 font-bold uppercase text-right w-24">Pieces</th>
                <th className="border-b border-r border-slate-200 px-3 py-2 font-bold uppercase text-right w-28">Value</th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="border-b border-r border-slate-200 px-3 py-1.5 text-right font-bold text-slate-700 bg-slate-50/30">
