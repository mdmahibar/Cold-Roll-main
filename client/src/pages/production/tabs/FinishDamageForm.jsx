...
const FinishDamageForm = ({ onAdd }) => {
  const [product, setProduct] = useState('Pasteurized Milk');
  const [unit, setUnit] = useState('LTR');
  const [quantity, setQuantity] = useState(0);
  const [saleRate, setSaleRate] = useState(0);

  const amount = (parseFloat(quantity) || 0) * (parseFloat(saleRate) || 0);

  const handleAdd = () => {
    if (quantity <= 0 || saleRate <= 0) return;
    onAdd({
      id: Date.now(),
      product,
      unit,
    ...
    <div className="flex flex-col rounded-sm border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
        <div className="flex flex-col gap-1 lg:col-span-2">
          <label className="text-[10px] font-bold uppercase text-slate-500">Finished Product</label>
          <select
            value={product}
          ...
          >
            ...
            <option value="Premium Paneer">Premium Paneer</option>
            <option value="Fresh Curd">Fresh Curd</option>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-bold uppercase text-slate-500">Sale Rate</label>
          <input 
            type="number"
            min="0"
            step="0.01"
...
export default FinishDamageForm;
