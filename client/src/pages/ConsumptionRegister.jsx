...
const mockData = [
  { date: '10/03/26', refNo: 'PI-2026-0005', type: 'Prod Issue', division: 'ICE CREAM', itemCode: 'MLK-BUF-001', itemDesc: 'Buffalo Milk (Raw)', category: 'Raw Milk', qtyConsumed: 3200, unit: 'KG', rate: 88.00, value: 281600, prodOrder: 'PO-PROD-005', batch: 'BATCH-0005' },
  { date: '10/03/26', refNo: 'PI-2026-0005', type: 'Prod Issue', division: 'ICE CREAM', itemCode: 'SGR-001', itemDesc: 'Sugar (Refined)', category: 'Ingredients', qtyConsumed: 320, unit: 'KG', rate: 80.00, value: 25600, prodOrder: 'PO-PROD-005', batch: 'BATCH-0005' },
  { date: '12/03/26', refNo: 'GI-2026-0012', type: 'Adhoc Issue', division: 'MILK', itemCode: 'CIP-CHM-001', itemDesc: 'CIP Chemical', category: 'Chemicals', qtyConsumed: 5, unit: 'KG', rate: 450.00, value: 2250, prodOrder: '-', batch: '-' },
  { date: '14/03/26', refNo: 'PI-2026-0006', type: 'Prod Issue', division: 'ICE CREAM', itemCode: 'MLK-BUF-001', itemDesc: 'Buffalo Milk (Raw)', category: 'Raw Milk', qtyConsumed: 4100, unit: 'KG', rate: 88.00, value: 360800, prodOrder: 'PO-PROD-006', batch: 'BATCH-0006' },
  { date: '14/03/26', refNo: 'PI-2026-0006', type: 'Prod Issue', division: 'ICE CREAM', itemCode: 'VAN-ESS-001', itemDesc: 'Vanilla Essence', category: 'Ingredients', qtyConsumed: 15, unit: 'LTR', rate: 890.00, value: 13350, prodOrder: 'PO-PROD-006', batch: 'BATCH-0006' },
  { date: '17/03/26', refNo: 'PI-2026-0007', type: 'Prod Issue', division: 'ICE CREAM', itemCode: 'SGR-001', itemDesc: 'Sugar (Refined)', category: 'Ingredients', qtyConsumed: 350, unit: 'KG', rate: 80.00, value: 28000, prodOrder: 'PO-PROD-007', batch: 'BATCH-0007' },
  { date: '20/03/26', refNo: 'GI-2026-0015', type: 'Adhoc Issue', division: 'MILK', itemCode: 'PKG-FOIL-001', itemDesc: 'Milk Packaging Foil', category: 'Packaging', qtyConsumed: 50, unit: 'KG', rate: 120.00, value: 6000, prodOrder: '-', batch: '-' },
];
...
const ConsumptionRegister = () => {
  ...
  const [valueField, setValueField] = useState('Qty Consumed');
  ...
  const dimensions = ['Item', 'Division', 'Month', 'Category', 'Production Order'];
  const valueFields = ['Qty Consumed', 'Value (₹)'];
  ...
  const keyMap = {
    ...
    'Month': 'date',
    'Category': 'category',
    'Production Order': 'prodOrder'
  };

  const valKeyMap = {
    'Qty Consumed': 'qtyConsumed',
    'Value (₹)': 'value'
  };
  ...
  const getVal = (row, field) => {
    ...
    if (field === 'Month') {
      ...
      return `${monthNames[monthIdx]} 20${parts[2]}`;
    }
    if (field === 'Division') {
  ...
  const groupedData = useMemo(() => {
    ...
    return Object.entries(groups).map(([groupName, items]) => {
      const subtotals = {
        qtyConsumed: items.reduce((sum, item) => sum + item.qtyConsumed, 0),
  ...
  const grandTotals = useMemo(() => ({
    qtyConsumed: mockData.reduce((sum, item) => sum + item.qtyConsumed, 0),
    value: mockData.reduce((sum, item) => sum + item.value, 0),
  }), []);

  const renderTypeBadge = (text) => {
    if (text === 'Prod Issue') {
      return <span className="inline-flex items-center px-2 py-0.5 rounded-full border border-blue-300 text-[9.5px] font-bold text-blue-600 bg-transparent">Prod Issue</span>;
    }
    if (text === 'Adhoc Issue') {
      return <span className="inline-flex items-center px-2 py-0.5 rounded-full border border-orange-300 text-[9.5px] font-bold text-orange-500 bg-transparent">Adhoc Issue</span>;
    }
    return text;
  };

  const renderRow = (row, i) => (
    <tr key={row.refNo + row.itemCode + i} className="hover:bg-blue-50/30 transition-colors">
      <td className="px-3 py-2.5 font-medium text-gray-600">{row.date}</td>
      <td className="px-3 py-2.5 font-bold text-sap-primary">{row.refNo}</td>
      <td className="px-3 py-2.5">{renderTypeBadge(row.type)}</td>
      ...
      <td className="px-3 py-2.5 text-right font-bold text-gray-700 tabular-nums">{fmtAmount(row.qtyConsumed)}</td>
      <td className="px-3 py-2.5 font-medium text-gray-600 text-[10px]">{row.unit}</td>
      <td className="px-3 py-2.5 text-right font-medium text-gray-600 tabular-nums">{fmtRate(row.rate)}</td>
      <td className="px-3 py-2.5 text-right font-bold text-gray-700 tabular-nums">{fmtAmount(row.value)}</td>
      <td className="px-3 py-2.5 font-medium text-gray-600">{row.prodOrder !== '-' ? row.prodOrder : <span className="text-gray-400">—</span>}</td>
      <td className="px-3 py-2.5 font-medium text-gray-600">{row.batch !== '-' ? row.batch : <span className="text-gray-400">—</span>}</td>
    </tr>
  );
    ...
    <div className="mx-auto flex max-w-[1440px] flex-col gap-3 pb-10">
      ...
      <div className="flex items-start justify-between">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-1.5 text-[11px] text-gray-400">
            ...
            <span>›</span>
            <span className="text-gray-600 font-medium">Consumption Register</span>
          </div>
          <div className="flex items-center gap-2.5">
            <BarChart2 className="h-5 w-5 text-gray-500" />
            <div>
              <h1 className="text-[17px] font-bold text-gray-800 tracking-tight">
                Periodic Consumption Register
              </h1>
              <p className="text-[11px] text-gray-500 mt-0.5 font-medium">
                Material consumption tracking · Division · Location · Item category filters
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-lg border border-[#cbe1f8] bg-[#f4f9ff] shadow-sm p-3">
        <div className="grid grid-cols-8 gap-3 items-end">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[9px] font-extrabold text-sap-primary uppercase tracking-wider">Consumption Type</label>
            <select className="form-select text-[11px] h-[30px] border-gray-300 rounded focus:border-sap-primary focus:ring-sap-primary text-gray-700 w-full bg-white">
              <option>All</option>
            </select>
          </div>
          ...
          <div className="col-span-2 flex gap-2 h-[30px]">
        ...
        <div className="overflow-x-auto">
          ...
          /* ── FLAT / GROUP TABLE ── */
            ...
            <table className="w-full text-left text-[11px] whitespace-nowrap">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  {['Date', 'Ref. No.', 'Type', 'Division', 'Item Code', 'Item Description', 'Category'].map(h => (
                  ...
                  <th className="px-3 py-2.5 font-extrabold uppercase tracking-wider text-gray-500 text-[9.5px] text-right">Qty Consumed <span className="text-gray-300 ml-0.5">↕</span></th>
                  ...
                  <th className="px-3 py-2.5 font-extrabold uppercase tracking-wider text-gray-500 text-[9.5px] text-right">Rate (₹) <span className="text-gray-300 ml-0.5">↕</span></th>
                  <th className="px-3 py-2.5 font-extrabold uppercase tracking-wider text-gray-500 text-[9.5px] text-right">Value (₹) <span className="text-gray-300 ml-0.5">↕</span></th>
                  <th className="px-3 py-2.5 font-extrabold uppercase tracking-wider text-gray-500 text-[9.5px]">Prod. Order <span className="text-gray-300 ml-0.5">↕</span></th>
                  <th className="px-3 py-2.5 font-extrabold uppercase tracking-wider text-gray-500 text-[9.5px]">Batch <span className="text-gray-300 ml-0.5">↕</span></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {viewMode === 'Flat' ? (
                  ...
                  groupedData && groupedData.map((group, idx) => (
                    <React.Fragment key={idx}>
                      ...
                      >
                        <td colSpan={13} className="px-3 py-2 text-[11px]">
                      ...
                      {!collapsedGroups[group.groupName] && (
                        <tr className="bg-[#f0f7ff] border-t border-[#cbe1f8]">
                          <td colSpan={7} className="px-3 py-2.5 font-bold italic text-sap-primary text-[11px]">↳ Subtotal</td>
                          <td className="px-3 py-2.5 text-right font-bold text-sap-primary text-[11.5px] tabular-nums">{fmtAmount(group.subtotals.qtyConsumed)}</td>
                          <td colSpan={2}></td>
                          <td className="px-3 py-2.5 text-right font-bold text-sap-primary text-[11.5px] tabular-nums">{fmtAmount(group.subtotals.value)}</td>
                          <td colSpan={2}></td>
                        </tr>
                      )}
                    </React.Fragment>
                ...
                <tr className="bg-[#eaf3fc] border-t-2 border-[#b5d5f5]">
                  <td colSpan={7} className="px-3 py-3 font-extrabold text-gray-800 text-[11.5px]">Grand Total</td>
                  <td className="px-3 py-3 text-right font-extrabold text-gray-800 text-[11.5px] tabular-nums">{fmtAmount(grandTotals.qtyConsumed)}</td>
                  <td colSpan={2}></td>
                  <td className="px-3 py-3 text-right font-extrabold text-gray-800 text-[11.5px] tabular-nums">{fmtAmount(grandTotals.value)}</td>
                  <td colSpan={2}></td>
                </tr>
              </tbody>
            </table>
...
export default ConsumptionRegister;
