...
/* Open dialogs, oldest first — see the ESC effect in Modal. */
const ESC_STACK = [];
...
/* Every dropdown in the app is the shared ReactSelect (see
   components/common/ReactSelect) — searchable, and the same control everywhere.
   The field markup around it stays exactly as it was so Modal.css keeps drawing
   the floating "notch" label and the error state, and so onChange still hands
   callers a native-looking { target: { name, value } }. */
...
/* Kept for the call sites that ask for it by name — SelectField is already
   searchable, so the two are the same control. */
const SearchableSelectField = SelectField;
...
export function Modal({
  ...
  /* ESC key — topmost dialog only.
     Modals nest (a RecordPicker opens on top of the document being edited),
     and every open one listens on `document`. Without the stack, one Escape
     would close the picker AND throw away the form underneath it. Each open
     dialog pushes a token on mount; only the last one in acts. */
  useEffect(() => {
