...
function TextField({
  ...
  return (
    <div className={`modal-field ${className} ${error ? " modal-field--error" : ""}`.trim()}>
      {label && (
      ...
      <div className="modal-input-wrap">
        ...
        <input
          ...
          required={required}
          autoFocus={autoFocus}
          className={`modal-input ${inputClass} ${prefix ? " has-prefix" : ""} ${suffix ? " has-suffix" : ""}`.trim()}
        />
...
export function Modal({
  ...
  /* layout */
  variant = "inline",     // "inline" (default) — plain full-width card in normal page flow,
  // no backdrop, no dimming, sidebar stays fully interactive
  // (matches the Collect Milk page). "overlay" — the old floating,
  // centered, backdrop-dimmed popup dialog.
  size = "lg",            // only used when variant="overlay": sm | md | lg | xl | full
  resizable = true,       // only used when variant="overlay"
  position = "center",    // only used when variant="overlay": center | top | right (side drawer)
  ...
  /* ── size map ── (overlay variant only — inline is always 100% width, auto height) */
  const isFull = isOverlay && size === "full";
  const sizeMap = {
    sm: { width: 480, height: 340 },
    md: { width: 600, height: 480 },
    lg: { width: 860, height: 640 },
    xl: { width: 1120, height: 760 },
  ...
  const initial = isFull ? { width: 0, height: 0 } : (sizeMap[size] ?? sizeMap.lg);
  ...
  useEffect(() => {
    if (size === "full") return;
    const map = { sm: { width: 480, height: 340 }, md: { width: 600, height: 480 }, lg: { width: 860, height: 640 }, xl: { width: 1120, height: 760 } };
    setSize(map[size] ?? map.lg);
  }, [size]);
  ...
  /* body scroll lock — only relevant for the overlay variant. The inline
     card sits in normal page flow, so the page must stay scrollable behind
     (and there IS no "behind" — the sidebar and table stay fully usable,
     matching the Collect Milk page). */
  useEffect(() => {
    if (!isOverlay) return;
    if (open) {
  ...
  /* ── shared inner markup (header / tabs / body / footer / resize handles) ── */
  const dialogInner = (
    <>
      {/* ── HEADER ── */}
      ...
      {/* ── RESIZE HANDLES ── (overlay variant only, and never on a full-screen or side-drawer dialog) */}
      {isOverlay && resizable && !isFull && position !== "right" && (
        <>
  ...
  /* ── INLINE variant (default) — a plain card in normal page flow.
     No fixed-position overlay, no backdrop, no dimming: the sidebar and
     rest of the page stay fully visible and interactive, exactly like the
     Collect Milk page's "Add Collection Entry" card. ── */
  if (!isOverlay) {
    return (
      <div
        ref={dialogRef}
        className="modal-dialog inline-form-card"
        role="dialog"
  ...
  /* ── OVERLAY variant (opt-in via variant="overlay") — the original
     floating, centered, backdrop-dimmed popup dialog. ── */
  return (
    <div
      ref={overlayRef}
